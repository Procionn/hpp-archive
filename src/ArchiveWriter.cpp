...
#ifdef __linux__
            archive_entry_set_pathname(entry, (pathInArchive / relative_path).string().c_str());
#elif WIN32
...
#ifdef __linux__
        archive_entry_set_pathname(entry, (pathInArchive / path.filename()).string().c_str());
#elif WIN32
...
void ArchiveWriter::open_file () {
#ifdef __linux__
    archive_write_open_filename(main, (path += expansion).string().c_str());
#elif WIN32
...
ArchiveWriter::ArchiveWriter (const std::filesystem::__cxx11::path& archivePath,
                              const std::filesystem::__cxx11::path& expansion,
                              std::function<void(archive*)> openFunc)
    : path(archivePath), expansion(expansion) {
    openFunc(main);
    open_file();
}
